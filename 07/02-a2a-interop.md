...
```mermaid
flowchart LR
    subgraph A2A["A2A 侧(历史设计)"]
        Card["Agent card"]
        Task["task send/get/cancel"]
        State["task state"]
    end
    subgraph AV["aevatar 侧"]
        Host["Host boundary 翻译"]
        EE["EventEnvelope / actor command"]
        ES["actor state + event store"]
    end
    Card --> Host
    Task --> EE
    State -.->|"不作权威"| ES
    classDef hist fill:#fde68a,stroke:#d97706,color:#451a03;
    class Card,Task,State hist;
```
...
flowchart LR
    Now["当前 HEAD"]
    Now --> Del["3 个 A2A 项目源码已删(8bfd8605c)<br/>目录只剩空壳 / 构建残留"]
    Del --> Dec{"维护者决策"}
    Dec --> O1["删除本篇"]
    Dec --> O2["标历史已放弃"]
    Dec --> O3["重新实现(需新设计 issue)"]
