...
最容易混的是 boundary / seam / port 这一组:

```mermaid
flowchart LR
    A["Actor 的边界(boundary)"] -->|"不是 seam"| X["✗ 不是可替换点"]
    A --> P["它周围的 Port(seam)<br/>IActorDispatchPort / IEventPublisher"]
    P -->|"这里才换实现"| OK["✓ Local ↔ Orleans 在 Port 处替换"]
    classDef seam fill:#dcfce7,stroke:#16a34a,color:#14532d;
    class P,OK seam;
```
...
Mattpocock `improve-codebase-architecture/LANGUAGE.md`、Michael Feathers《Working Effectively with Legacy Code》、canon `overview.md` / `architecture.md` / `cqrs-projection.md`。
