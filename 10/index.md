...
> 诚实记录当前实现里**已确认、可复现**的问题与边界限制。各篇按事故发生时与当前复核目的分别声明自己的精确基线,给出**复现 → 根因 → 影响面 → 规避/修复方向**四段,并明确区分"按设计的限制"与"真正的 bug"。
...
| [07](07-scheduled-task-not-firing.md) | 定时任务为什么不触发 | 重激活从 now 重算跳过到期拍(`ac325cc70`)/ Orleans-Garnet 脑裂冻结(配置非代码)/ 历史 provision 塞两个凭证源违反「恰好一个」(`625e64c7e`,当前模型已替代) | 历史真 bug 已修 + 环境 |
...
| [12](12-api-security-audit-and-hardening.md) | API 安全审计与加固 | OAuth2/JWT、持有证明、明文密钥、租户隔离与端点授权的系统审计 | 两项 HIGH + 分阶段加固路线 |

> 另见 [09/03 全链路附录(§5 live 实测发现)](../09/03-provision-and-observe-via-nyxid/01-end-to-end.md)——CC/Codex 经 NyxID provision + 观测 workflow 时活体暴露的 6 条发现(binding 读模型最终一致曾 `500`、绑定慢异步流水线撞网关 `499`、serving revision 未自动激活、当前 C1 每 fire exchange 后以 Vault reference 交给 run 复用、demo run 验证 C2、`WorkflowExecutionCurrentStateDocument` ES 字段超限)。①~③属于早期同步实现历史;当前 C1 不 poll、不 direct invoke。
