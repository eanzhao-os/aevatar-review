...
- **计数时效**：本章所有数字（181 个 slnx 项目、9 个过滤器、src 106 / test 51 / agents 24 个 `.csproj`、各 `.slnf` 项目数）都是 `f02aa690` 基线的实测快照，由章末证据映射中的命令生成。它们描述规模量级，不构成架构承诺；任何后续 commit 都可能改变它们。结构切换前的仓库地图曾沿用 src 98 个项目、agents.slnf 3 个项目等旧计数，与本基线实测不符，应以本章为准。
