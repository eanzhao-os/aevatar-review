...
> ⚠️ **边界声明**:`POST /api/chat` 在 Mainnet Host 的 `README.md` 被标注为"不再是 `aevatar app` 的官方运行契约"(scope-first `/api/scopes/{scopeId}/...` 是当前 app 契约)。但 `/api/chat` 作为**框架层 workflow 能力**仍由 `Aevatar.Workflow.Host.Api` 提供,是理解 aevatar workflow 运行态最快的一条路,所以本篇 Quick Start 用它。后续 `01/02-chat-api-and-sse.md` 会展开 scope-first 与 chat 的关系。
...
|---|---|---|---|
| 本地开发 / CLI / Workflow.Host | 本地文件 store(`~/.aevatar/secrets.json`) | 加密文件(AES-256-GCM,master key 在 macOS Keychain 或 `~/.aevatar/masterkey.bin`) | `AevatarSecretsStore` 第 1-10、114-151 行 |
...
默认 provider 名由 `LLMProviders:Default` 决定(`AevatarSecretsStore` )。
...
> 这两个变量会被 `AddEnvironmentVariables("AEVATAR_")` 捕获(`AevatarConfigLoader` ),`AEVATAR_` 前缀被剥掉后即标准配置路径。Mainnet Host 用同样的环境变量机制,只是**不允许**本地 `secrets.json` 文件回退。
...
`AddAevatarMainnetHost()`(`MainnetHostBuilderExtensions` )做了大量装配:强制顺序启动 hosted service(注释引用了一次 2026-06-03 的 CrashLoopBackOff 事故)、加载分布式 Orleans、组合 platform / tool provider / 认证等。本地开发不需要全部理解,先跑起来。
...
端口来自 `MainnetHostBuilderExtensions` :`LocalDevelopmentListenUrl = "http://127.0.0.1:5080"`。`ConfigureMainnetListenUrls`()在非容器环境下用这个 URL,除非你用 `ASPNETCORE_URLS` 或 `--urls` 覆盖。

启动成功后,日志里会看到 `Now listening on: http://127.0.0.1`。
...
发请求(`POST /api/chat`,协议见 `chat-api` 第 19-29、41-67 行):
...
`curl -N` 关闭缓冲,让你实时看到 SSE 帧。请求体字段(`chat-api.md` ):
...
事件类型常量在 `WorkflowRunEventTypes` 这是 SSOT。`GetEventType`()把 proto `WorkflowRunEventEnvelope.EventOneofCase` 一一映射到这些常量。
...
```
> 运行上下文快照(`chat-api.md` 的 CUSTOM 子类型)。携带 actorId / commandId / projection 信息。
...
```
> token 用量(`WorkflowRunEventTypes` 这个常量在 `chat-api.md` 的清单里没列,但代码里有)。
...
```
> 运行收敛后的状态快照。CQRS 通用交互服务在 run 收敛后通过 `WorkflowRunFinalizeEmitter` 发出(见 `llm-streaming` )。
...
同样的能力也走 WebSocket(`GET /api/ws/chat`,`chat-api.md` )。客户端发 `{ "type": "chat.command", "requestId": "...", "payload": { "inputParts": [...] } }`,服务端依次回 `command.ack` → 若干 `agui.event`(每个对应一个 SSE 帧)→ 结束。文本帧和二进制帧都支持,回复帧类型匹配入站帧类型。

---

## 第 4 步:看运行报告

每次 workflow run 结束后,`FileSystemWorkflowRunReportExporter`(`FileSystemWorkflowRunReportExporter` )会在 `<repo>/artifacts/workflow-executions/` 写两个文件:
...
文件名规则见 `WorkflowRunReportExportWriter` :`workflow-execution-<yyyyMMdd-HHmmss>.{json,html}`,时间戳是运行时间。

- **JSON**(`WorkflowRunReportExportWriter` ):camelCase 缩进序列化的 `WorkflowRunReport`,字段含 `WorkflowName`、`CommandId`、`Success`、`DurationMs`、`Usage`(tokens/model/cost/latency)、`Summary`(步骤统计)、`Steps`(每步详情)、`Timeline`(事件时间线)等。
- **HTML**(`BuildHtml`,):自包含暗色主题页面,标题 `Workflow Execution Report`,带 Overview / Summary / Input / Final Output / Topology / Steps / Role Replies / Timeline 等区块。
...
>
> 这是**文件侧报告**,和投影侧的 `WorkflowRunInsightReportArtifactProjector`(`WorkflowRunInsightReportArtifactProjector` )是两回事:前者是本地文件,后者是 CQRS 读侧 artifact。两者关系会在 `05/04-workflow-projection.md` 展开。
...
1. Mainnet Host 默认监听哪个端口?(答:`http://127.0.0.1`,`MainnetHostBuilderExtensions` )
2. Mainnet Host 能不能读本地 `secrets.json`?(答:不能,强制 `AllowLocalFileSecretsStore = false`)
3. `simple_qa` 的 YAML 长什么样?(答 个 role + 1 个 `llm_call` step,`workflows/simple_qa.yaml`)
4. `TEXT_MESSAGE_CONTENT` 之前和之后分别是哪个帧?(答:`TEXT_MESSAGE_START` → `TEXT_MESSAGE_CONTENT`×N → `TEXT_MESSAGE_END`,`WorkflowRunEventTypes` )
5. 运行报告写在哪?(答:`<repo>/artifacts/workflow-executions/workflow-execution-<timestamp>.{json,html}`,`FileSystemWorkflowRunReportExporter` )
