...
| Workflow tool approval resume | 冻结基线描述"迟到 resume 是 no-op"；HEAD（`origin/feature/integrate`，以 feature/integrate checkout 核验为准）对 tool approval resume 收紧为显式拒绝：identity 不匹配则保留 pending 并提交 `WorkflowToolApprovalResumeRejectedEvent`，timeline/Observatory 以 `tool_approval_resume_rejected` 呈现；顶层 executionId/toolCallId/approvalRequestId 是无效别名返回 400 | `c8f46a3c6`（Reject invalid workflow approval resumes）；[03/05 warning](../03/05-pause-signal-approval-and-resume.md) | re-baseline 时改写 03/05 迟到 resume 语义：区分 human approval（仍 no-op）与 tool approval（显式拒绝 + 拒绝事件）；错误码与负向测试同步 |
