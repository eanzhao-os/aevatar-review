...
> 当前 C1 与凭证时序核对基线:`origin/feature/integrate @ 4e0def2c231b7074209b852b855954b3db7d3e71`。C1 最初上线于 `c80c77929`,C2 上线于 `bd9975c8a`;第 5 节保留 2026-06-19 早期实现的活体事故,但不再把当时的同步 poll/invoke 流程描述成当前请求栈。
...
flowchart LR
    ...
    C1 -->|"admit + resolve/create + async bind"| MEMBER["Team workflow member"]
    C1 -->|"cron / runImmediately"| SCHEDULE["Workflow-kind schedule"]
    SCHEDULE -->|"cron 到点尝试 dispatch<br/>target ready 才产 run"| WF
...
- aevatar 收到后,把这个 `sub` 映射成内部的 **`scope_id`**——也就是该调用者在 aevatar 里能看见/能操作的隔离边界(参见已知问题 [10/01](../../10/01-cli-lark-scope-isolation.md) 的同一套 `scope_id` 隔离口径)。
...
C1 把“校验 capability、找到或创建 member、接受 bind、按需建立 schedule”收口成一条调用:`POST /api/scopes/{scopeId}/provision-workflow`。当前流程始终非阻塞:

1. **admit before mutation**:先解析 workflow capability,按 `RunImmediately/Cron` 选择 interactive 或 durable execution mode;invalid/non-ready 时不创建 member 或 schedule。
2. **resolve/create member**:用 `(scopeId, teamId, displayName)` 派生稳定 provision identity;已存在则复用,不存在才创建 workflow member。`memberId`、`workflowId` 与 `publishedServiceId` 各自保持独立语义。
3. **accept bind**:调用 `BindAsync`,记录 `bindingRunId`,但**不轮询 `GetBindingRunAsync`**。binding 是可持续数分钟的异步流水线。
4. **optional ensure schedule**:只有 `Cron` 非空或 `RunImmediately=true` 才创建 Workflow-kind schedule。schedule **不等待 binding readiness**;每个 cron occurrence 到点就尝试 dispatch,只有 published service 已可用时才产生 run,否则记录本拍失败并推进到下一个 occurrence。recurring 使用调用方 cron;`RunImmediately` 合成近未来固定月日时分的 5-field demo cron,若首拍早于分钟级 binding 完成,下一 occurrence 会是下一年同月日时分,并不存在短周期自动重试。没有 cron 且不立即运行时是诚实的 bind-only。

端点成功时始终返回 `202 Accepted`,表示 member/bind/可选 schedule 已受理,不表示 binding 完成或 run 已完成。响应包含 `memberId`、`bindingRunId`、可选 `scheduleId`、Studio/Observatory URL;校验或 admission 失败返回 `400`。
...
sequenceDiagram
    ...
    participant CAP as Capability Admission
    participant MEM as StudioMemberService
    participant SCH as ScheduledDispatch
    CC->>EP: POST /api/scopes/{scopeId}/provision-workflow
    EP->>SVC: ProvisionAsync(spec)
    SVC->>CAP: Admit/Revalidate(workflow, executionMode)
    CAP-->>SVC: typed admission plan
    SVC->>MEM: Get deterministic member
    alt member 不存在
        SVC->>MEM: CreateAsync(kind=workflow, teamId)
        MEM-->>SVC: memberId + publishedServiceId
    else 已存在
        MEM-->>SVC: memberId + publishedServiceId
    end
    SVC->>MEM: BindAsync(memberId)
    MEM-->>SVC: bindingRunId
    Note over SVC,MEM: 不 poll,不 direct invoke
    alt Cron 或 RunImmediately
        SVC->>SCH: EnsureAsync(deterministic schedule)
        SCH-->>SVC: scheduleId
    else bind-only
        Note over SVC,SCH: no schedule, no run
    end
    SVC-->>EP: accepted response
    EP-->>CC: 202 Accepted
```

> **为什么是「一次调用」而不是让 CC 自己串多步**:调用方不需要理解 capability admission、member identity、binding 与 schedule generation;服务端用 deterministic identity 让重试收敛。非阻塞 `202` 则把“请求已受理”和“binding/run 已完成”分开,避免把分钟级异步流水线塞进一次 HTTP。

---

## 4. C2:平台级只读 Observatory,浏览器 OIDC + 近实时轮询

当可选 schedule 成功产出 run 后,怎么「看着它跑」?C2 = 一个**平台级、只读、scope 隔离**的 Workflow Run Observatory(`src/workflow/Aevatar.Workflow.Infrastructure/CapabilityApi/WorkflowRunObservatoryEndpoints.cs`),设计上有几个硬约束(端点注释自陈):
...
- **鉴权在端点层,不在 query service**:跨 scope 只读查询是一条**独立窄契约** `IWorkflowRunAdminQueryService.ListAllRunsAsync`,它**自己不鉴权**——鉴权由端点在调用它**之前**强制;原 scope-bound 的 `IWorkflowRunObservatoryQueryService` 一行没改。把"跨 scope"做成新契约、而非给老契约加 `isAdmin` 参数,正是为了让默认隔离路径**不可能**因多一个布尔参数被误绕过(FI-005 边界优先于便利)。
...
## 5. 附录:2026-06-19 live 发现与当前凭证重核

> ①~③、⑤~⑥来自 2026-06-19 的 aevatar mainnet 活体执行:mock 把异步、网关、凭证或 ES 投影简化后,这些问题不会出现。④保留当时 `llm_call` 凭证失败的现象,但其当前机制解释已在 2026-07-24 按 `origin/feature/integrate @ 4e0def2` 重新追踪,不是对当前 C1 的第二次 live canary。

!!! note "这一节是历史事故,不是当前请求时序"
    ①~③描述的是曾经同步 poll/invoke 的版本。当前 `4e0def2` 已删除 request-path polling 与 direct invoke,成功统一返回 `202`,run 由可选 schedule 产生。保留这些记录是为了说明为什么当前 contract 必须非阻塞。
...
> **后续修复(`ef52fc532`)**:激活本身还藏着一个**投影竞态**——`ActivateServiceRevisionCommand` 读的 revision-catalog 是**滞后于** `CreateRevision/PrepareRevision` 提交的投影;prepared artifact 还没物化时它抛**不可重试**的 `InvalidOperationException`(运行时只重试可恢复的并发冲突),激活就**终态性地输掉这场竞态**:serving set 永不写、member 卡在 `ServingSetMissing`、永不可调用。线上表现为「**新 provision 的定时 workflow member 永远不产 run**,而早先碰巧赢得竞态的 member 能跑」——正是这种「有时行有时不行」最难查。修复把激活改成**有界 self-continuation**(prepared artifact 还看不见、且未 `PreparationFailed` 时,按命令上携带的 `activation_deadline_at` 短延迟重投 `ActivateServiceRevisionCommand`,直到投影追上),不引入 app 层阻塞轮询、保住非阻塞 provision 路径。这条与本附录①「读模型最终一致」同根:**绑定/激活链路上的每一次读模型读取都要容忍投影滞后窗口**。

### ④ C1 每次 fire 先换一张短票;它不等于 Member Automation Agent Key

2026-06-19 的 demo 已经产出 run,随后在 `llm_call` 暴露 caller credential/config 未齐,NyxID provider 以 `NyxIdAuthenticationRequiredException` 拒绝。当前 `4e0def2` 已不是当时那条请求时序:one-call C1 `/provision-workflow` 保存 `SenderNyxId` subject,但没有带 `BindingId` 的 `CallerAuthority`;所以每次 fire 都由 schedule dispatch 先调用 NyxID exchange 换一张短票。dispatch 随后把 raw token 写入 Vault 的 `WorkflowCallerDurableBearerToken` purpose,只把 `DurableCallerCredentialRef` 交给 workflow;run 内的 LLM/tool/connector consumer 在外呼前解析并复用这张票,不会各自重新 mint。broker、binding、scope、exchange provider 或首次 Vault store 失败时,fire 会在 workflow dispatch 前 fail closed,不会创建 run;只有成功投递后 reference 在消费期变得不可解析时,既存 run 才会在外呼处因无 credential 失败。

> **演进更正**:`4a028ff07` / `0d7fb928` 曾尝试把 durable bearer/key 直接建模为 schedule source,但该模型已被后续 contract 淘汰。`durable_sender_bearer_token` 现在只允许读取历史事件,新写入会 fail closed。当前 C1 的 fire-time short ticket 虽然也短暂进入 Vault,但它是 dispatch 生成的 **workflow caller projection**,不是重新写回 schedule state 的 legacy durable bearer。dedicated Agent Key 则只在另一条 canonical Team Member Automation API 落地,见 [02 生产闭环](02-scheduled-agent-key-production-canary.md);不能拿 02 的结果替 C1 背书。
...
- **附录区分 2026-06-19 live 现象与 2026-07-24 当前凭证重核**:①~③是历史修复/结论,④保留当时现象并重核当前 C1,⑤是当时验证实况,⑥是旁观到的独立 infra 问题。把 canonical Member Automation Agent Key、C1 fire-time exchange 与⑥混在一起都会误导排障。
- **§4.1 管理员跨 scope 视图未活体亲验**:它按端点 `WorkflowRunObservatoryEndpoints` + `NyxIdPlatformAdminAuthorizer` 源码解读(鉴权矩阵、fail-closed、kill-switch、审计均有源码对应),但**尚未**用一个真实 admin token 在活体上跑 `scope=__all__` 并核对审计落盘;第 5 节的 2026-06-19 live 现象早于这次 C2 扩展,2026-07-24 只重核了④的当前凭证代码。
...
> **读者应能回答**:为什么 CC/Codex 不需要给 aevatar 写专用 MCP server 或专用 agent(§0/§1/§2)?入站「当大脑」为什么必须是无状态网关、它和 [方案 02](../02-ingress-tool-ownership/index.md) 的工具所有权问题怎么同源(§1)?当前 C1 接受哪四段工作、为什么不 poll/invoke、`202` 表示什么、什么时候才创建 schedule(§3)?C2 凭什么做到「平台级只读 + scope 隔离 + 无前端部署」(§4)?平台管理员要跨 scope 看 run 时,默认隔离是怎么**不被削弱**地开一道口子的——身份在哪里验、为什么做成独立契约而非 `isAdmin` 参数、怎么 fail-closed/可 kill/留痕(§4.1)?附录 6 条里,哪几条是 mock 单测测不出、只有活体才暴露,各自的根因是什么(§5)?
