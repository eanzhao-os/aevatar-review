...
5. 使用 `--dry-run` 和普通手动运行验证扫描链路。
6. 从仓库模板生成本机 plist，校验后通过 `launchctl bootstrap` 加载。
7. 使用 `launchctl kickstart`、`launchctl print`、日志和状态文件完成验收。
...
- **误创建 issue**：安装验收优先使用 `--dry-run`；普通运行的副作用在命令前说明。
