...
**Goal:** Add a repository-local skill that safely fetches Aevatar `origin/feature/integrate`, prepares a frozen evidence bundle, updates or extends the Chinese book, and requires independent rotating review before advancing the documented baseline.

**Architecture:** Keep semantic decisions in `.agents/skills/updating-aevatar-review-docs/SKILL.md` and deterministic Git, mapping, sampling, and state transitions in one Python 3 standard-library helper. Reuse the existing frozen-snapshot and documentation validators, while making the committed update state the single baseline consumed by local and CI gates.
...
- `prepare` may fetch and create a derived frozen snapshot, but it must not modify review documents, create GitHub issues, or advance state.
- Only `commit-state` may advance `documented_upstream_sha`, and only with an exact prepared fact bundle, an explicit successful-gates flag, and every sampled old chapter marked reviewed.
...
- Create `.config/aevatar-doc-update/state.json`: initial documented baseline and per-chapter review counts.
- Modify `.gitignore`: ignore only this skill's `.superpowers/aevatar-doc-update/` runtime directory.
- Modify `scripts/check-md.sh`: read its default SHA and verification date from committed state.
...
- Modify `scripts/git-hooks/pre-push`: materialize and validate the committed state SHA instead of a hard-coded SHA.
- Modify `.github/workflows/docs.yml`: read the committed state SHA before checking out the frozen upstream.
...
write(review / ".config/aevatar-doc-update/state.json", json.dumps({
    "schema_version": 1, "documented_upstream_sha": baseline,
...
- Produces CLI command `init-state --state PATH --plan PATH --baseline-sha SHA --verified-at YYYY-MM-DD --completed-at ISO8601`.
...
- State schema contains `schema_version`, `documented_upstream_sha`, `documented_verified_at`, `last_successful_update_at`, and one `review_count`, `last_reviewed_sha`, `last_reviewed_at`, `result` record per active substantive chapter.
...
class StateAndSamplingTests(CliFixture):
    def test_init_state_records_every_plan_chapter_at_zero(self):
        ...
        self.assertEqual(value["documented_upstream_sha"], SHA_A)
        self.assertEqual(value["documented_verified_at"], "2026-07-25")
...
- Produces `prepare --review-root PATH --upstream-repo PATH --state PATH --map PATH --snapshot-script PATH --snapshot-root PATH --branch feature/integrate --verified-at YYYY-MM-DD --sample-size 6 --exclude-chapter PATH --output PATH`.
...
def test_same_target_keeps_documented_verified_date(self):
    self.remote_integrate_sha = self.baseline_sha
    facts = self.prepare(verified_at="2026-08-02")
    self.assertEqual(facts["verified_at"], "2026-07-25")
...
def test_commit_state_advances_atomically_after_all_evidence(self):
    ...
    self.assertEqual(value["documented_upstream_sha"], facts["target_sha"])
...
Also test that a missing baseline object and a changed state hash both fail without rewriting state.
...
Capture `HEAD` and `status --porcelain=v1` before fetch; require both to be byte-for-byte equal afterward. Resolve the target from `refs/remotes/origin/feature/integrate^{commit}`. Require the baseline object with `cat-file -e SHA^{commit}`. Detect ancestry with `merge-base --is-ancestor`; for normal history list `baseline..target`, and for rewritten history list `target --not baseline`. In both cases compute final changed paths using `git diff --name-status -M baseline target`. Preserve merge commits and every subject; apply no prefix filtering.
...
Enumerate candidates deterministically from the frozen snapshot:
...
`commit-state` validates the selected facts, snapshot marker, current state SHA-256, and new-chapter issue map; requires `--gates-passed`; and requires the reviewed set to equal the union of `semantic_changed_chapters` and `review_sample`. It reparses the current `PLAN.md`, drops state rows for deleted chapters, initializes newly listed chapters at count zero, increments only sampled old chapters, sets their target SHA/time/result, updates `documented_upstream_sha`, `documented_verified_at`, and the global completion time, and atomically replaces state. It never edits chapter Markdown. When target equals baseline, `prepare` uses the state's existing `documented_verified_at`, and commit preserves it while updating only completion and review fields.
...
- Consumes: state keys `documented_upstream_sha` and `documented_verified_at`.
- Produces: local and CI defaults derived from the same committed state; `EXPECTED_UPSTREAM_COMMIT` and `EXPECTED_VERIFIED_AT` remain explicit override hooks for an in-progress update before state advances. Active chapter membership and status come from `PLAN.md`; the 2026-07-25 migration manifest stays immutable historical evidence.

- [ ] **Step 1: Make the fixture validator test demand state-driven defaults**

In the `validators` fixture inside `scripts/tests/test-doc-checks.sh`, create a fixture `PLAN.md` with the good and planned chapters, plus `.config/aevatar-doc-update/state.json` with the fixture SHA, `documented_verified_at: 2026-07-25`, and a completion timestamp. Run `check-md.sh` without `EXPECTED_UPSTREAM_COMMIT`. Add five assertions: changing the state SHA makes the fixture chapter fail with `frontmatter upstream_commit must be ...`; adding a completed chapter row only to `PLAN.md` makes `--all` accept it instead of calling it orphan; `check-links.py --allow-planned` accepts an unwritten path listed only in `PLAN.md`; `check-drift.sh` applies its `设计待论证` check to a new chapter listed only in `PLAN.md`; and if `PLAN.md` contains three completed rows while an active README claims two substantive chapters, drift fails with a count mismatch. Restore fixture state between assertions. Add `aevatar-doc-update` to the usage text and `all` loop so the Python suite runs through:
...
if [ -z "${EXPECTED_UPSTREAM_COMMIT:-}" ] || [ -z "${EXPECTED_VERIFIED_AT:-}" ]; then
  [ -f "$STATE_FILE" ] || { echo "check-md: missing documented baseline state: $STATE_FILE" >&2; exit 1; }
  state_values="$(python3 - "$STATE_FILE" <<'PY'
import json, re, sys
value = json.load(open(sys.argv[1], encoding="utf-8"))
sha = value.get("documented_upstream_sha", "")
verified_at = value.get("documented_verified_at", "")
if not re.fullmatch(r"[0-9a-f]{40}", sha) or not re.fullmatch(r"\d{4}-\d{2}-\d{2}", verified_at):
    raise SystemExit("invalid documented baseline state")
...
python3 .agents/skills/updating-aevatar-review-docs/scripts/prepare-update.py init-state \
  ...
  --plan PLAN.md \
  --baseline-sha f02aa690bbebb9cabeac30a553d737486b0eb661 \
  --verified-at 2026-07-25 \
  --completed-at 2026-07-25T00:00:00Z
```
...
assert value['documented_upstream_sha'] == 'f02aa690bbebb9cabeac30a553d737486b0eb661'
assert value['documented_verified_at'] == '2026-07-25'
assert len(value['chapters']) == 72
assert {row['review_count'] for row in value['chapters'].values()} == {0}
PY
```

- [ ] **Step 5: Make pre-push and CI consume the committed state**

In `scripts/git-hooks/pre-push`, replace the hard-coded SHA with a fail-closed JSON read of `documented_upstream_sha`; keep the existing `AEVATAR_UPSTREAM_REPO` override and snapshot materialization.

In `.github/workflows/docs.yml`, add this step before the upstream checkout:

```yaml
      - name: Read documented upstream baseline
        id: baseline
        run: |
          python3 - <<'PY' >> "$GITHUB_OUTPUT"
          import json, re
          value = json.load(open('.config/aevatar-doc-update/state.json', encoding='utf-8'))
          sha = value['documented_upstream_sha']
          verified_at = value['documented_verified_at']
...
Use `${{ steps.baseline.outputs.sha }}` as the second checkout's `ref`. Materialize `HEAD` into `$RUNNER_TEMP/aevatar-frozen`, write that directory to `$GITHUB_ENV` as `AEVATAR_SRC`, and run the chapter gate with both explicit baseline outputs. Remove every active workflow occurrence of the old literal SHA.
...
Expected: all tests pass and the final `rg` prints no matches. Then commit only the listed files:
...
- 全程以本轮固定的 `origin/feature/integrate` SHA 冻结快照为唯一代码事实源。
...
python3 .agents/skills/updating-aevatar-review-docs/scripts/prepare-update.py prepare \
  ...
  --branch feature/integrate --verified-at "$(date +%F)" \
...
目标 SHA 变化时，所有普通章节 frontmatter 和公共版本入口必须迁到同一 SHA/date；逐项判定正文中的旧 SHA、计数和结论。block index 不增加 SHA/date frontmatter，但其中的事实与计数必须同步。仅 frontmatter 的机械迁移不算语义修订。优先对真正受影响的最少章节修订正文、事实源入口、图、示例、状态、索引与导航。新增 feature 只有在形成独立职责、协议或读者问题时才成章。
...
用 facts 的 snapshot、target SHA 和 verified date 运行：

```bash
AEVATAR_SRC="$TARGET_SNAPSHOT" EXPECTED_UPSTREAM_COMMIT="$TARGET_SHA" EXPECTED_VERIFIED_AT="$VERIFIED_AT" bash scripts/check-md.sh --all
...
- 每轮必须由一个全新上下文的独立 agent 复核本轮变更和默认 6 篇轮转旧正文；未通过不得推进文档基线。
```
...
Expected: both diffs are empty; the facts target equals current `origin/feature/integrate`; committed state remains at `f02aa690bbebb9cabeac30a553d737486b0eb661` because this is prepare-only. Do not update actual chapters or create issues during this forward test.
